# Multi-stage Dockerfile for MC-Kanban
# Stage 1: Build client
FROM node:20-alpine AS client-builder

WORKDIR /app

# Copy package files and vendor tarball
COPY package*.json ./
...
# Install dependencies (design system from vendor tarball)
...
# Copy all package files and vendor tarball
COPY package*.json ./
COPY server/package*.json ./server/
COPY client/package*.json ./client/
COPY shared/package*.json ./shared/
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD wget --no-verbose --tries=1 --spider http://localhost:${PORT:-3049}/api/health || exit 1
