...
const APP_SLUG = process.env.APP_SLUG || 'kanban';
...
export const requireAuth = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (appAccess.role_slug) {
      req.user.role = appAccess.role_slug;
    }
  } catch (error) {
    return res
      .status(403)
      .json({ error: { message: error instanceof Error ? error.message : 'Access denied' } });
  }
